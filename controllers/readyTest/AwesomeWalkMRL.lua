...
footSizeX = Config.walk.footSizeX or {-0.075, 0.08}
...
function update()
    ...
    if (iStep > iStep0) then
        ...
        dx1 =
            (aXP * math.exp(tStep / tZmp) - aXN * math.exp(-tStep / tZmp)) / tZmp +
            m2X * (1 - math.cosh((1 - ph2Zmp) * tStep / tZmp))
        dy1 =
            (aYP * math.exp(tStep / tZmp) - aYN * math.exp(-tStep / tZmp)) / tZmp +
    ...
    if initial_step > 0 then
        zFoot = 0
    end --Don't lift foot at initial step
    pLLeg[3], pRLeg[3] = 0, 0
    if supportLeg == 0 then -- Left support
        if current_step_type > 1 then --walkkick
            if xFoot < walkKickPh then
                uRight = util.se2_interpolate(xFoot * 2, uRight1, uRight15)
            else
                uRight = util.se2_interpolate(xFoot * 2 - 1, uRight15, uRight2)
            end
        else
        ...
        if current_step_type > 1 then --walkkick
            if xFoot < walkKickPh then
                uLeft = util.se2_interpolate(xFoot * 2, uLeft1, uLeft15)
            else
    ...
    --Turning
    local turnCompX = 0
    if math.abs(velCurrent[3]) > turnCompThreshold and velCurrent[1] > -0.01 then
        turnCompX = turnComp
    end
    ...
    uTorsoActual = util.pose_global(vector.new({-footX + frontCompX + turnCompX + armPosCompX, armPosCompY, 0}), uTorso)
    ...
    mcm.set_motion_supportLeg(supportLeg)
...
function check_stepkick()
    ...
    if stepKickRequest == 1 then
        ...
        if supportLeg == stepKickSupport then
            if (step_check_count > 2) or --      ( math.abs(uFootErr[1])<0.02 and
                    --        math.abs(uFootErr[2])<0.01 and

                    (math.abs(uFootErr[1]) < 0.02 and math.abs(uFootErr[2]) < 0.01 and
...
function check_walkkick()
    ...
    if walkKickRequest == 1 then
        ...
        uFootErr = util.pose_relative(uLeft1, util.pose_global(2 * uLRFootOffset, uRight1))
        if
            supportLeg ~= walkKick[1][3] or math.abs(uFootErr[1]) > 0.02 or math.abs(uFootErr[2]) > 0.01 or
                math.abs(uFootErr[3]) > 10 * math.pi / 180
         then
            if supportLeg == 0 then
                uRight2 = util.pose_global(-2 * uLRFootOffset, uLeft1)
            else
    ...
    if #walkKick[walkKickRequest] <= 7 then
        ...
        if supportLeg == 0 then
            ...
            uRight2 = util.pose_global({footPos1[1], footPos1[2] - 2 * footY, footPos1[3]}, uLeft1)
        else
            uLeft2 = util.pose_global({footPos1[1], footPos1[2] + 2 * footY, footPos1[3]}, uRight1)
        end
    else
        footPos1 = walkKick[walkKickRequest][7]
        footPos2 = walkKick[walkKickRequest][8]
        if supportLeg == 0 then
            uRight15 = util.pose_global({footPos1[1], footPos1[2] - 2 * footY, footPos1[3]}, uLeft1)
            uRight2 = util.pose_global({footPos2[1], footPos2[2] - 2 * footY, footPos2[3]}, uLeft1)
        else
            uLeft15 = util.pose_global({footPos1[1], footPos1[2] + 2 * footY, footPos1[3]}, uRight1)
            uLeft2 = util.pose_global({footPos2[1], footPos2[2] + 2 * footY, footPos2[3]}, uRight1)
        end
    end
...
function update_still()
    ...
    uTorsoActual = util.pose_global(vector.new({-footX + armPosCompX, armPosCompY, 0}), uTorso)
...
function motion_legs(qLegs, gyro_off)
    if mcm.get_debug_qLegUpdated() == 0 then
        --print(mcm.get_debug_qLegUpdated())
        mcm.set_debug_qLegs(qLegs)
        mcm.set_debug_qLegUpdated(1)
    end
...
function update_velocity()
    if velCurrent[1] > velXHigh then
        ...
        velDiff[1] = math.min(math.max(velCommand[1] - velCurrent[1], -velDelta[1]), velDeltaXHigh)
    else
        velDiff[1] = math.min(math.max(velCommand[1] - velCurrent[1], -velDelta[1]), velDelta[1])
    end
    velDiff[2] = math.min(math.max(velCommand[2] - velCurrent[2], -velDelta[2]), velDelta[2])
    velDiff[3] = math.min(math.max(velCommand[3] - velCurrent[3], -velDelta[3]), velDelta[3])
...
function doStepKickLeft()
    if stepKickRequest == 0 then
        ...
        support_start, support_end = largestep.set_kick_type('nonstop_kick_left')
        stepKickRequest = 1
...
function doStepKickRight()
    if stepKickRequest == 0 then
        mcm.set_walk_isStepping(1)
        support_start, support_end = largestep.set_kick_type('nonstop_kick_right')
        stepKickRequest = 1
        stepKickSupport = support_start
    end
end
...
function advanceMotion()
    ...
    if ph > 1 then --Advance frame
        if #current_motion == motion_index then
            ...
            motion_start_time = t
            qLArmOR0[1], qLArmOR0[2], qLArmOR0[3] = qLArmOR1[1], qLArmOR1[2], qLArmOR1[3]
            qRArmOR0[1], qRArmOR0[2], qRArmOR0[3] = qRArmOR1[1], qRArmOR1[2], qRArmOR1[3]
            bodyRot0[1], bodyRot0[2], bodyRot0[3] = bodyRot1[1], bodyRot1[2], bodyRot1[3]
...
function stance_reset() --standup/sitdown/falldown handling
    ...
    elseif Motion.sm:get_previous_state()._NAME == 'PunchKick' then
        --------------------------------------------------------------------------------------------------------------
        print('Kick Resetted')
        if mcm.get_kick_type() == 'kickForwardLeft' then
            iStep0 = -1
            iStep = 0
        else
            iStep0 = 0
...
function foot_phase(ph)
    ...
    c8 = 0.1142
    --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    if use_alternative_trajectory == 1 then
        ph1FootPhase = 0.1
        ph2FootPhase = 0.5
        ph3FootPhase = 0.8
    ...
    elseif use_alternative_trajectory == 2 then
        if ph <= 0.42 then
            zf = 0.5 * (1 - math.cos(2 * math.pi * phSingleSkew))
        else
            zf =
                a1 * math.exp(-((ph - b1) / c1) ^ 2) + a2 * math.exp(-((ph - b2) / c2) ^ 2) +
                a3 * math.exp(-((ph - b3) / c3) ^ 2) +
                a4 * math.exp(-((ph - b4) / c4) ^ 2) +
                a5 * math.exp(-((ph - b5) / c5) ^ 2) +
                a6 * math.exp(-((ph - b6) / c6) ^ 2) +
                a7 * math.exp(-((ph - b7) / c7) ^ 2) +
                a8 * math.exp(-((ph - b8) / c8) ^ 2)
        end
        ph1FootPhase = 0.1
        ph2FootPhase = 0.5
        ph3FootPhase = 0.8
